...
// }
	...
	var targetObj = {
		url : approot + '/data?module=before&service=TransNoticeMine&method=getNoticeOtherTargetData',
		columns : [ {
			display : '宗地编号',
			name : 'name',
			width : 200,
	...
	//
	targetObj.url = approot + '/data?module=before&service=TransNoticeMine&method=getNoticeTargetData&noticeId=' + pageObj.noticeId;
	targetObj.toolbar = {
		items : [ {
			text : '增加标的',
	...
	//
	var bankObj = {
		url : approot + '/data?module=before&service=TransNoticeMine&method=getOtherBankList',
		columns : [ {
			display : '收款单位',
			name : 'organ_name',
			width : 150
...
// }
	...
	bankObj.url = approot + '/data?module=before&service=TransNoticeMine&method=getNoticeBankList&noticeId=' + pageObj.noticeId;
	bankObj.toolbar = {
		items : [ {
			text : '增加银行',
...
pageObj.refresh = function() {
	//
	var url = approot + '/data?module=before&service=TransNoticeMine&method=getNoticeTargetData&noticeId=' + pageObj.noticeId;
	pageObj.boundedTargetGrid.refresh(url, {});
	//
	url = approot + '/data?module=before&service=TransNoticeMine&method=getNoticeBankList&noticeId=' + pageObj.noticeId;
...
pageObj.boundBanks = function() {
	...
	var url = approot + '/data?module=before&service=TransNoticeMine&method=getOtherBankList';
...
pageObj.initAttachList = function() {
	...
	var initHeight = 200;
	var grant = {};
	if(pageObj.noticestatus>=2 && pageObj.noticestatus!=4 ){
