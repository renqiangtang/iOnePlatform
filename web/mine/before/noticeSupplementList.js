...
pageObj.initPage = function() {
	var url = approot + '/data?module=before&service=TransNoticeMine&method=noticeTreeList&u='+ pageObj.u + "&goodsType=" + pageObj.goodsType;;
	pageObj.grid = $('#grid').ligerGrid({
				url : url,
				columns : [{display : '补充公告名称',name : 'name',width : '20%',render : pageObj.nameRender}, 
				           {display : '补充公告号',name : 'no',width : '18%'}, 
				           {display : '公告类型',name : 'notice_type',width : '8%',render : pageObj.renderNoticeType}, 
				           {display : '交易类型',name : 'business_type_name',width : '8%'}, 
...
pageObj.query = function() {
	...
	var url = approot+ '/data?module=before&service=TransNoticeMine&method=noticeTreeList&goodsType=' + pageObj.goodsType;
