...
pageObj.init = function() {
	...
	cmd.table = "trans_business_type_rel where goods_type_id in ('301','401') ";
	var html = '';
...
pageObj.allStatistics=function(targetId){
	DialogModal({url: approot + '/mine/after/allStatistics.html',
...
pageObj.renderStatus = function(row, rowNumber) {
	...
	if (is_stop == 1) {
		status = comObj.targetStatusObj[status] + '<font color="red">(终止)</font>';
	} else if (is_suspend == 1) {
		status = comObj.targetStatusObj[status] + '<font color="red">(中止)</font>';
	}else{
