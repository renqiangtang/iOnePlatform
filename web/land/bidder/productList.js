...
pageObj.exeQuery = function() {
	...
	cmd.success = function(data) {
		...
		for ( var i = 0; i < arr.length; i++) {
			...
			if (obj.is_bid == 1) {
				html += '	<span class="f-gm" title="直接购买" onclick="pageObj.addApply(\'' + obj.id + '\',\'' + obj.notice_name + '\',\'' + obj.no + '\',\'' + obj.allow_trust + '\',\'' + obj.business_type + '\',\'' + obj.trans_type_id + '\');"></span>';
...
pageObj.addApply = function(targetId, noticeName, targetName,allowTrust,businessType,transTypeId) {
...
pageObj.initPage = function() {
	...
	cmd.success = function(data) {
		...
		if(typeof(arr)!="undefined"){
			for ( var i = 0; i < arr.length; i++) {
				if(arr[i].canton_id!=null && arr[i].canton_id!="null" && arr[i].canton_id!=""){
					html += '<div><a id="canton' + arr[i].canton_id + '" type="canton" href="javascript:pageObj.query(\'canton' + arr[i].canton_id + '\');">' + arr[i].canton_name + '</a></div>';
				}
			}
			$('#cantonDiv').html(html);
		}
