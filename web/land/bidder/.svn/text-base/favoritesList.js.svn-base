...
pageObj.showDetail = function(row, detailPanel, callback) {
	...
	var goodsSize = row.goods_size ? row.goods_size+"平方米" : '';
	...
	if (transType == 2 || (transType == 1 && isNetTrans == 0)) {
		...
		detailBody = '<div class="i-arrow"></div><table class="i-l-gridGetails" style="width:98%">' + '<tr><td width="120" height="28" class="tr">起始价：</td><td width="150" class="fb">' + beginPrice + priceUnit
				...
				+ goodsSize + '</td><td class="tr">竞买申请开始时间：</td><td class="fb">' + beginApplyTime + '</td><td height="28" class="tr">集中报价期开始时间：</td><td class="fb">' + beginFocusTime + '</td></tr>';
		if (transType == 0 && isNetTrans == 0) {
			...
			detailBody = detailBody + '<tr><td class="tr">集中报价期截止时间：</td><td class="fb">' + endFocusTime + '</td><td class="tr">限时竞拍开始时间：</td><td class="fb">' + beginLimitTime + '</td></tr>';
		} else {
