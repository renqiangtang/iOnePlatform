...
pageObj.operateRender = function(row, rowNamber) {
	...
	+ "\")'>入账申请单</a>&nbsp;&nbsp;";
	if (status >= 1) {
		operate += "<a id='btnCert' href='javascript:pageTradeCommonObj.viewCert(\"" + licenseId + "\")'>资格确认书</a>&nbsp;&nbsp;";
	}
...
//			+ licenseId+"&bidderName=&targetName="+row.targetName+"'>交易记录证书文件</a>&nbsp;&nbsp;";
	...
	if ((targetStatus ==7||targetStatus ==5) && (status == 3 || status == 4)) {
		operate += "<a id='btnTransCert' href='javascript:pageTradeCommonObj.viewTransCertNotice(\"" + licenseId + "\")'>网上竞得证明</a>&nbsp;&nbsp;";
	}
	if (targetStatus > 3) {
		operate += "<a id='btnViewOfferLog' href='javascript:pageTradeCommonObj.viewLicenseOfferLog(\"" + licenseId + "\")'>出价记录</a>&nbsp;&nbsp;";
	}
	if (targetStatus <= 4 && status == 0 && payEarnest == 0 && isUnion == 0) {
		var unionLicense = unionCount > 0 ? 1 : 0;
		operate += "<a id='btnDeleteLicense' href='javascript:pageObj.deleteLicense(\"" + licenseId + "\", " + unionLicense + ")'>删除</a>&nbsp;&nbsp;";
	}
