...
tradeL03Obj.html_refresh = function() {
	...
	cmd.executeAsync();
	//错误数据包
//	if (tradeCommon.pretargetId) {
//		for ( var i = 0; i < 10; i++) {
//			var cmd = new Command();
//			cmd.module = tradeMain.module;
//			cmd.service = "Trade";
//			cmd.method = "getTradeInfo";
//			cmd.targetId = tradeCommon.pretargetId;
//			cmd.success = function(data) {
//				if (data.node && data.node.ptype && data.node.ptype == "L03")
//					tradeL03Obj.html_load(data);
//			};
...
tradeL03Obj.html_load = function(data) {
	...
	tradeCommon.finalTip = data.node.finalTip;
	//	
	...
	$('#L03tNo').html(targetHref+tradeCommon.reserveInfo(data));
	...
	if (data.node.isEnd == 1) {
		...
		case -1:
			...
			break;
		case 0:
			$('#L03choiceMsg').html("本地块已达到最高地价" + "[" + data.node.finalValueStr + "]，将转入竞配建阶段,是否同意继续出价?");
			$('#L03iAgree').show();
			$('#L03iNoAgree').show();
			break;
		case 1:
			$('#L03choiceMsg').html("还剩下" + data.node.restTime + "进入第二指标（竞配建）竞价!");
			$('#L03iAgree').hide();
			$('#L03iNoAgree').hide();
			break;
		}
