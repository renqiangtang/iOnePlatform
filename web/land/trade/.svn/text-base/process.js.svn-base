...
//启动流程引擎
//---------------------------------
pageProcessObj.startEngine101 = function() {
	var cmd = new Command();
...
pageProcessObj.startEngine301 = function() {
	var cmd = new Command();
...
pageProcessObj.startEngine501 = function() {
	var cmd = new Command();
	cmd.module = "tradeplow";
	cmd.service = "Trade";
	cmd.method = "startEngine";
	cmd.success = function(data) {
	   DialogAlert('启动完成！');
...
pageProcessObj.initGrid = function(){
	pageProcessObj.gridManager = $("#processGrid").ligerGrid({
		url: approot + '/data?module=tradeland&service=Trade&method=activeProccess&moduleId=' + pageProcessObj.moduleId,
		columns: [
...
pageProcessObj.queryData = function () {
	...
	var url = approot + '/data?module=tradeland&service=Trade&method=activeProccess&moduleId=' + pageProcessObj.moduleId;
...
pageProcessObj.reload = function(){
	pageProcessObj.gridManager.loadServerData( approot + "/data?module=tradeland&service=Trade&method=activeProccess&moduleId=" + pageProcessObj.moduleId);
}
...
$(document).ready(function(){
	...
	//pageProcessObj.queryEngineState();
	//启动流程引擎
	$('#btnStart101').click(function() {
		 pageProcessObj.startEngine101();
	});
	$('#btnStart301').click(function() {
		 pageProcessObj.startEngine301();
	});
	$('#btnStart501').click(function() {
		 pageProcessObj.startEngine501();
	});
	//刷新
	...
	$('#btnMonitor').click(function() {
		var obj={};
		obj.url=approot+'/land/trade/tradeMonitor.html';
