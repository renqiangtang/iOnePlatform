...
tradeL02Obj.html_load = function(data) {
	...
	$('#L02tNo').html(targetHref+tradeCommon.reserveInfo(data));
	...
	if (data.node.isEnd == 1) {
		...
		case -1:
			...
			break;
		case 0:
			$('#L02choiceMsg').html("本地块已达到最高地价" + "[" + data.node.finalValueStr + "]，将转入竞配建阶段,是否同意继续出价?");
			$('#L02iAgree').show();
			$('#L02iNoAgree').show();
			break;
		case 1:
			$('#L02choiceMsg').html("还剩下" + data.node.restTime + "进入竞配建指标竞价!");
			$('#L02iAgree').hide();
			$('#L02iNoAgree').hide();
			break;
		}
