	...
	cmd.success = function(data) {
		if (data.state == 1) {
			...
			if (pageObj.targetExists) {
				...
				$("#transType").html(Utils.getRecordValue(data, 0, 'trans_type_name'));
				...
				if (pageObj.targetStatus == 5 || pageObj.targetStatus == 6 ||  pageObj.targetStatus==7) {
					...
					if (pageObj.targetStatus == 5) {
						$("#transPrice").html(Utils.getRecordValue(data, 0, 'trans_final_price'));
						$("#landPrice").html(pageObj.convertFloatValue(Utils.getRecordValue(data, 0, 'unit_price')));
						$("#bulidPrice").html(pageObj.convertFloatValue(Utils.getRecordValue(data, 0, 'building_price')));
...
pageObj.loadTargetImages = function() {
	if (!pageObj.targetImagesLoaded) {
		...
		var url = approot + "/land/trade/viewImages.html?imgUrlRoot=" + imgRoot + "&imgUrl=" + imgs;
...
pageObj.loadTargetOfferLog = function() {
	if (!pageObj.targetOfferLogLoaded) {
		$("#subD").html("<iframe width='100%' height='710px' frameborder='0' id='offerLog' src='" + approot
				+ "/land/trade/offerLogList.html?hideQueryCondition=no&targetId=" + pageObj.targetId + "&gridWidth=965&gridHeight=700'></iframe>");
