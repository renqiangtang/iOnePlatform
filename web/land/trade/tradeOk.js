...
tradeOkObj.html_load = function(data) {
	...
	//var targetHref = '<a href="javascript:pageTradeCommonObj.viewTarget(\'' + tradeCommon.targetId + '\')" class="fb link01">' + tradeCommon.targetName + '</a>'
	...
	$('#OktargetName').html(targetHref+tradeCommon.reserveInfo(data));
...
tradeOkObj.viewTransCertNotice = function(){
	DialogOpen({
		height : 620,
		width : 800,
		url : approot + '/hint.html?licenseId='+tradeCommon.licenseId,
		title : '重要提示',
		buttons : [ {
			text : '关闭',
			onclick : function(item, dialog) {
