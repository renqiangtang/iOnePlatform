 ...
 */
public class TransNoticeDao extends BaseDao {
	...
	public ParaMap getNoticeTargetData(String noticeId,String page , String pagesize) throws Exception {
		ParaMap keyData = new ParaMap();
		keyData.put("moduleNo", "trans_prepare_notice_manage");
	...
	public ParaMap checkNotice(String noticeIds , String userId,int isCon,int isNoticeCheck) throws Exception{
		...
		if(StrUtils.isNotNull(noticeIds)){
			...
			if(ids!=null && ids.length > 0){
				for(int i = 0 ; i < ids.length ; i ++){
					if(isNoticeCheck==0){
						keyData.clear();
						keyData.put("id", noticeIds);
						keyData.put("is_valid", 1);
						keyData.put("status", 5);
						ParaMap updateMap=dataSetDao.updateData("trans_notice", "id", keyData);
						if(updateMap.getInt("state")!=1){
					...
					if(isCon==2){//发布
						if(status!=5){//进行发布，但是，不是已审核的公告是不允许发布的
							throw new Exception(shtext+"公告失败！必须审核通过的公告才能发布！");
						}
					}else{//审核
						if(status!=3){//审核必须是状态为3的公告
...
//	 * @throws Exception
//	 */
//	public ParaMap doCheckNotice(String noticeId , String userId)throws Exception{
//		DataSetDao dataSetDao = new DataSetDao();
//		//审核
//		ParaMap keyData = new ParaMap();
//		keyData.put("id", noticeId);
//		keyData.put("status", 1);
//		keyData.put("check_user_id", userId);
//		keyData.put("check_date", DateUtils.getStr(DateUtils.now()));
//		keyData.put("status", 1);
//		ParaMap formatMap = new ParaMap();
//		formatMap.put("check_date", "to_date(?, 'yyyy-mm-dd hh24:mi:ss')");
//		ParaMap out = dataSetDao.updateData("trans_notice", "id", keyData,formatMap);
//		return out;
//	}
//	
	/**
...
//	}
	...
	/**
	 * 发布公告
	 * @param sqlParams isZd 0自动提交,1手动提交
	 * @throws Exception
	 */
	public ParaMap publishNotice (String noticeIds,String userId,int isCon,int isZd) throws Exception {
		if(isZd!=0){//1手动提交的，去验证是否需要审核 ，0自动提交则不需要验证，因为前面已经验证过了
			...
			if(isCheckNotice==0){
				throw new Exception( "管理员已经设置公告不需要审核,可直接发布！");
			}
		}
		ParaMap out = new ParaMap();
		DataSetDao dataSetDao = new DataSetDao();
...
//		}
	...
	public ParaMap transNormalPublishNoticeList(String organId , String goodsType)throws Exception{
		...
		if(StrUtils.isNotNull(goodsType)){
			ParaMap customConditions = new ParaMap();
			customConditions.put("CUSTOM_CONDITION", " and exists (select * from trans_notice_target_rel tntr , trans_target tt where tntr.target_id = tt.id and substr(tt.business_type,1,3)='"+goodsType+"' and tt.is_stop=0 and tt.is_suspend=0 and tntr.notice_id = tn.id ) ");
	...
	public ParaMap noticeTreeList(String organId , String goodsType) throws Exception{
		DataSetDao dataSetDao = new DataSetDao();
		ParaMap sqlParams = new ParaMap();
		sqlParams.put("moduleNo", "trans_supplement_manage");
		...
		if (StrUtils.isNotNull(goodsType)) {
			customCondition.append(" and  substr(tn.business_type, 1, 3) = :goodsType ");
	...
	public ParaMap getSupplementNoticeTargetData(String noticeId , String parentId) throws Exception {
		ParaMap keyData = new ParaMap();
		keyData.put("moduleNo", "trans_supplement_manage");
