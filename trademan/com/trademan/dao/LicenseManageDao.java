 ...
 * 2012-05-11
 */
public class LicenseManageDao extends BaseDao {
	...
	public ParaMap getConfirmLicenseListData(String moduleId, String dataSetNo, ParaMap sqlParams) throws Exception {
		if (StrUtils.isNull(moduleId))
			sqlParams.put("moduleNo", "trans_license_confirm_plow");
		else
			sqlParams.put("moduleId", moduleId);
		if (StrUtils.isNull(dataSetNo))
			...
			if (result.getInt("state") == 1 && result.getInt("totalRowCount") > 0) {
				...
				if (result.getInt("state") == 1 && result.getInt("totalRowCount") > 0) {
					...
					if (result.getInt("state") == 1) {
						...
						if(result.getSize()>0){
							int sfunion=result.getRecordInt(0, "isunion");
							if(sfunion>0){
								result.clear();
								result.put("state", 0);
	...
	public ParaMap confirmUnionLicense(String moduleId, String dataSetNo, ParaMap licenseData) throws Exception {
		...
		if (!confirmed.equals("1") || result.getInt("state") == 1) {
			...
			//新需求，联合人中只要有一个拒绝了联合，那么其它联合人在确认联合的时候弹出提示
			if(confirmed.equals("1")){
				keyData.clear();
				keyData.put("license_id", licenseId);
				keyData.put("status",2);
				result.clear();
				result = dataSetDao.querySimpleData("trans_license_union", keyData);
				if(result.getInt("state")==1){
					if(result.getInt("totalRowCount")>0){
						StringBuffer mess=new StringBuffer("您的这个竞买申请中,由于有联合竞买人：\n");
						for(int i=0;i<result.getInt("totalRowCount");i++){
							if(i==result.getInt("totalRowCount")-1){
								mess.append(result.getRecordString(i, "bidder_name"));
							}else{
								mess.append(result.getRecordString(i, "bidder_name")+",");
							}
						}
						mess.append(" \n拒绝了联合竞买,此联合竞买无效。请重新竞买申请！");
						
						
						result.clear();
						result.put("state", 0);
						result.put("message", mess.toString());
						return result;
					}
				}
			}else{
...
//				result = dataSetDao.querySimpleData("trans_license_union", keyData);
//				if(result.getInt("state")==1){
//					if(result.getInt("totalRowCount")==0){//点拒绝的时候发现是第一个拒绝的
//						//将此竞买申请标识为无效
//						keyData.clear();
//						keyData.put("id", licenseId);
//						keyData.put("is_valid", 0);;
//						ParaMap reMap=dataSetDao.updateData("trans_license", "id", keyData);
//						if(reMap.getInt("state")!=1){
//							throw  new Exception("设置竞买申请无效失败！");
//						}
//					}
//				}
			}
	...
	public ParaMap getLicenseTransCertData(String moduleId, String dataSetNo, String id) throws Exception {
		...
		if (queryData.getInt("state") == 1) {
			if (queryData.getInt("totalRowCount") == 0) {
				...
				result.put("bidderTell", queryData.getRecordValue(0, "bidder_tel"));
				result.put("beginListTime", queryData.getRecordValue(0, "begin_list_time"));
				result.put("endListTime", queryData.getRecordValue(0, "end_list_time"));
				result.put("gsName", queryData.getRecordValue(0, "gs_name"));
				...
				String a="";
				for(int i = 0; i < queryData.getRecords().size(); i++) {
					a = a+ (String) queryData.getRecordValue(i, "bidder_name")+",占"+queryData.getRecordValue(i, "percent")+"%的土地使用权份额;";
				...
				result.put("unionBidderName" , a);
				ParaMap goodsParamMap = new ParaMap();
				ParaMap goodsData = new ParaMap();
				if (moduleId == null || moduleId.equals("") || moduleId.equalsIgnoreCase("null"))
					goodsParamMap.put("moduleNo", "trans_license_list");
				else
				...
				for(int i = 0; i < goodsData.getRecords().size(); i++) {
					if(goodsData.getRecords().size()==1){
						land_area = (String) goodsData.getRecordValue(i, "land_area") + "平方米";
						build_area = (String) goodsData.getRecordValue(i, "build_area") + "平方米";
					}else{
						if(i==0){
							land_area = (String) goodsData.getRecordValue(i, "land_area") + "平方米";
							build_area = (String) goodsData.getRecordValue(i, "build_area") + "平方米";
						}else{
							land_area = land_area + ";" + (String) goodsData.getRecordValue(i, "land_area") + "平方米";
							build_area = build_area + ";" + (String) goodsData.getRecordValue(i, "build_area") + "平方米";
						}
					}
				}
				result.put("land_area",land_area);
				result.put("build_area",build_area);
		return result;
	}
	...
	public ParaMap deleteLicenseData(String moduleId, String dataSetNo, String id) throws Exception {
		...
		if (result.getInt("state") == 1) {
			TransLicenseDao transLicenseDao = new TransLicenseDao();
	 ...
	 * @throws Exception
	 */
	public ParaMap confirmLicenseAfterFh(ParaMap confirmData) throws Exception {
		String userId = confirmData.getString("u");
		String id = confirmData.getString("id");
		String targetId = confirmData.getString("targetId");
		int confirmStatus = confirmData.getInt("confirmStatus");
		int afterCheck= confirmData.getInt("afterCheck");//为-1则为审批不通过，只有审批不通过的只需要复核
		String confirmOpinion = confirmData.getString("confirmOpinion");
		ParaMap result = new ParaMap();
		if(afterCheck!=-1){
			result.put("state", 0);
			result.put("message", "复核失败！竞买申请资格审批不通过的才能复核");
			return result;
		}
		if (StrUtils.isNull(targetId)) {
			result.put("state", 0);
		...
		if(confirmStatus == 7){
			...
			abort_reson = this.getOkBidder(targetId)+"复核不通过";
			after_check = -2;
		}
		DataSetDao dataSetDao = new DataSetDao();
		ParaMap paraData = new ParaMap();
