...
import com.portal.dao.QueryMineDao;

public class QueryMineService extends BaseService {
	/**
	 * 查找标的
	 * 
	 * @param inMap
	...
	public ParaMap getTransTargetList(ParaMap inMap) throws Exception {
	    ...
	    String moduleNo = "trans_mine_portal";
	    String dataSetNo = "";
	    if(tabId==0){
		...
		QueryMineDao queryDao = new QueryMineDao();
		...
		if(tabId==0){
			...
			String blackGoodsType="301";
		    sqlmap.put("goods_type", blackGoodsType);
...
//		String organId = getOrganId(inMap.getString("u"));
		...
		if (inMap.containsKey("goods_use")
			...
			if (StrUtils.isNotNull(goods_use)) {
				sqlParams.put("tgoods_use", "%"+goods_use+"%");
			}else{
				sqlParams.put("tgoods_use", "%%");
			}
		}else{
			sqlParams.put("tgoods_use", "%%");
		}
