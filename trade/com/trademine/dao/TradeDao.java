...
import com.trademine.engine.RandomUtils;
...
public class TradeDao extends BaseDao {
	...
	public ParaMap getTargets(String userId, String targetId, String goodsType)
		...
		// String sql =
		// "	select distinct tt.id as targetid,tt.name as targetname,tt.no as targetno,wn.name,wn.ptype,wn.qtype ,wtu.right_type as righttype,tt.status"
		// +
		// " from wf_process wp " +
		...
		// " and wtu.type = ? " +
		// " and wtu.value = ? " +
		// " order by  abs((sysdate - decode(tt.trans_type,0,tt.begin_focus_time,1,tt.begin_limit_time,tt.begin_focus_time ))*24*60*60) asc "
		// ;
		DataSetDao dataSetDao = new DataSetDao();
		ParaMap sqlParams = new ParaMap();
		sqlParams.put("moduleNo", "trans_trade");
		sqlParams.put("dataSetNo", "query_active_targets");
		// sqlParams.put("status",
		// Constants.ProcessRunning+","+Constants.ProcessEnd);
	...
	public String virtualTrade(ParaMap inMap) throws Exception {
		...
		String[] goodsUseArray = new String[] { "金", "银", "铜", "铁", "锡", "铝",
				"石油", "煤炭", "天然气", "地热", "铀", "镭" };
	...
	public String createVirtualTarget(ParaMap inMap) throws Exception {
		...
		sql.append(");");
		// 插入交易物记录
		sql.append("   insert into trans_goods(id,goods_type, no, name,goods_use,canton_id) values(new_goods_id, '301',?, ? , ?,?);");
		// 插入标的交易物关联
		...
		pstm.setString(1, targetID);
		pstm.setString(2, "测试矿产" + nowStr);
		pstm.setString(3, "测试矿产" + nowStr);
		...
		pstm.setBigDecimal(12, null);// fianl_price
		pstm.setBigDecimal(13, beginPrice);// reserve_price

		pstm.setString(14, "测试矿产" + nowStr);
		pstm.setString(15, "测试矿产" + nowStr);
	...
	public void createVirtualNotice(String targetIds) throws Exception {
		ResultSet rs = null;
		PreparedStatement pstm = null;
		...
		String targets = "'" + targetIds.replaceAll(";", "','") + "'";
		String sqlTransAccount = "select id from trans_account where business_type in (select business_type from trans_target where id in ("
				+ targets + "))";
		pstm = getCon().prepareStatement(sqlTransAccount);
		rs = pstm.executeQuery();
		while (rs.next()) {
