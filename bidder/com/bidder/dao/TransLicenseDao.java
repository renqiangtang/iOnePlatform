...
public class TransLicenseDao extends BaseDao {
	...
	public ParaMap isCanBuy(String bidderId , String targetId )throws Exception{
		...
		if(licenseMap.getSize()>0){
			List licenseIdList = new ArrayList();
			for(int i = 0 ; i < licenseMap.getSize() ; i ++ ){
				String sql="select tlu.id from trans_license_union tlu left join trans_license tl on tlu.license_id=tl.id where tlu.is_valid=1 and tlu.status=0 and tl.id=:tlId and tlu.bidder_id=:bidderId";
				keyData.clear();
				keyData.put("tlId",(String)licenseMap.getRecordValue(i, "id"));
				keyData.put("bidderId",bidderId);
				ParaMap validunionMap = dataSetDao.queryData(sql, keyData);
				
				//验证联合人中是否有拒绝的人,查出有记录，代表这个订单中有人拒绝
				keyData.clear();
				keyData.put("license_id", (String)licenseMap.getRecordValue(i, "id"));
				keyData.put("is_valid", 1);
				keyData.put("status", 2);
				ParaMap vnolicenseUnionMap = dataSetDao.querySimpleData("trans_license_union", keyData);
				if(validunionMap.getSize()>0 && vnolicenseUnionMap.getSize()<1){//没有处理的订单，且此订单没有任何一个联合人拒绝，则不允许再竞买
					out.put("state", 0);
					out.put("message", "您已被联合申请购买了该标的,请点击我的订单处理联合购买请求。");
					return out;
				}
				licenseIdList.add((String)licenseMap.getRecordValue(i, "id"));
			}
			...
			keyData.clear();
			keyData.put("bidder_id", bidderId);
			keyData.put("license_id", licenseIdList);
			keyData.put("is_valid", 1);
			keyData.put("status", 1);
			ParaMap licenseUnionMap = dataSetDao.querySimpleData("trans_license_union", keyData);
			...
			keyData.put("is_valid", 1);
			keyData.put("status", 2);
			ParaMap nolicenseUnionMap = dataSetDao.querySimpleData("trans_license_union", keyData);
			
			
			if(licenseUnionMap.getSize()>0 && nolicenseUnionMap.getSize()<1){
				out.put("state", 0);
		...
		if(targetMap.getSize()>0){
			...
			Date beginLimitTime = DateUtils.getDate(String.valueOf(targetMap.getRecordValue(0, "begin_limit_time")));
			...
			if((DateUtils.now()).getTime() > beginLimitTime.getTime()){
				out.put("state", 0);
				out.put("message", "该标的已经开始限时竞价了。");
				return out;
			}
	 ...
	 * @throws Exception
	 */
	public ParaMap getBidders(List unitProportionsList,String goodsType)throws Exception{
		ParaMap out = new ParaMap();
		...
		for(int i = 0 ; i < unitProportionsList.size(); i++){
			...
			if((","+bidderNames+",").indexOf(","+name+",")>=0){
				...
				if(result.getSize()>0){
					...
					ParaMap isBlackMap=new ParaMap();
					isBlackMap.put("goodsType", goodsType);
					isBlackMap.put("name", name);
					isBlackMap.put("is_company", -1);
					ParaMap blackMap=us.checkBlackUserByName(isBlackMap);
					if(blackMap.getInt("state")!=1){
						out.put("state", 0);
					    out.put("message", "联合竞买人"+name+"已经被列入黑名单，不允许联合竞买!");
				    	return out;
					}else{
	...
	public ParaMap getCorp(List unitProportionsList)throws Exception{
		...
		if(StrUtils.isNotNull(notGetName)){
		    ...
		    out.put("message", "贵方拟成立项目公司的股东应与竞买申请人一致，请重新填写，具体应与工商部门预核准文件内容相符。");
		    return out;
		}	
		if(StrUtils.isNotNull(repeatName)){
		    out.put("state", 0);
		...
		if(StrUtils.isNotNull(percentErrorName)){
			out.put("state", 0);
			if(unitProportionsList.size()>1){
				out.put("message", "贵方所录入的联合体成员所占土地使用权份额比例不足100%，请重新填写。");
			}else{
				out.put("message", "贵方所录入的项目公司股东的投资比例不足100%，请重新填写。");
			}
	    	return out;
		}
		if(StrUtils.isNull(bidderNames)){
...
//	}
	 ...
	 * @throws Exception
	 */
	public synchronized ParaMap saveLicense(String bidderId , String targetId , String trustType , String trustPrice , String userId ,int is_after_check ,int unitStyle) throws Exception{
		ParaMap out=new ParaMap();
...
//		out = dao.updateData("trans_license", "id", transLicenseMap, format);
		...
		if (versionMode != null && versionMode.equalsIgnoreCase("demo")){// 演示版本
			if(unitStyle==1){
				sql = "update trans_license set earnest_money_pay = 1 where id = '"+id+"'";
				pstm = getCon().prepareStatement(sql);
				pstm.executeUpdate();
			}else{
				sql = "update trans_license set status = 1 where id = '"+id+"'";
				pstm = getCon().prepareStatement(sql);
				pstm.executeUpdate();
	 ...
	 * @throws Exception
	 */
    public ParaMap updateCartDtl(String bidderId , String targetId , int begin , int end , String userId)throws Exception{
		 ...
		 String dtlId = null;
		 ParaMap cartDtlMap=new ParaMap();
		 if(result.getSize()>0){
			 dtlId = (String)result.getRecordValue(0, "id");
			 ketSet.clear();
			 ketSet.put("id", dtlId);
			 ketSet.put("status", end);
			 cartDtlMap=dao.updateData("trans_cart_dtl", "id", ketSet);
		 }else{
			 ketSet.put("id", null);
			 ketSet.put("cart_id", cartId);
			 ketSet.put("target_id", targetId);
			 ...
			 ParaMap formatMap = new ParaMap();
			 formatMap.put("create_date", "to_date(?, 'yyyy-mm-dd hh24:mi:ss')");
			 cartDtlMap=dao.updateData("trans_cart_dtl", "id", ketSet,formatMap);
		 }
		 if(cartDtlMap.getInt("state")==1){
			 cartDtlMap.put("cart_dtl_id", dtlId);
		 }
		 return cartDtlMap;
	}
	...
	public ParaMap getTransTarget(String targetId) throws Exception {
	  ...
	  public BigDecimal getTargetEarnest(String targetId , String currency)throws Exception{
		    ParaMap out = new ParaMap();
	    	ParaMap sqlParam = new ParaMap();
	    	sqlParam.put("target_id", targetId);
	    	sqlParam.put("is_valid", 1);
	    	sqlParam.put("currency", currency);
	    	DataSetDao dao = new DataSetDao();
	    	ParaMap result= dao.querySimpleData("trans_target_earnest_money", sqlParam);
	    	if(result.getSize()>0){
	    		return new BigDecimal(result.getRecordString(0, "amount"));
	    	}else{
	    		return null;
	    	}
		}
